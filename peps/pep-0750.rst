...
In particular, ``t'{value=}'`` is treated as ``t'value={value!r}'``; when
a ``conversion`` is not specified, it defaults to ``r``:
...
If an explicit conversion is specified, it is kept: ``t'{value=:s}'``
is treated as ``t'value={value!s}'``. 

If a format string is specified without a conversion, no default conversion 
is applied: ``t'{value=:fmt}'`` is treated as ``t'value={value:fmt}'``; 
``conversion`` is set to ``None`` in the resulting ``Interpolation``.
